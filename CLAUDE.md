...
A **framework-agnostic, pip-installable, detection-first drift guard for AI
agent memory.** It solves one concrete problem: long-running agents drift off
their purpose (a billing assistant starts answering about recipes and weather).
PurposeGuard scores every memory write against a declared purpose and surfaces
drift as a single watchable number.

v0.1 is **already built and passing 10 tests.** Your job is to extend it along
the roadmap below — not to redesign it.
...
- v0.2 (poisoning, NEXT): reference = **existing-memory consensus**.

Same core, scorer, policy, and meter. Only the reference changes. This is what
makes both capabilities reachable without a rewrite. If you find yourself
forking the core to add v0.2, stop — you're doing it wrong.

## Non-negotiable guardrails

1. **Detection-first.** v0.1 NEVER emits `Decision.BLOCK`. Enforcement is always
   opt-in and post-v0.1. There is a test (`test_never_blocks_in_v01`) enforcing
...
5. **Honesty in docs.** This is a heuristic reliability guardrail, NOT a proof
   and NOT a security boundary. Never describe it as security in v0.1 (that
   framing belongs to v0.2 poisoning + the experimental provenance module).
